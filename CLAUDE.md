...
- **`AdminPage`** — Super Admin only; lists all users in a table; `CreateUserModal` handles role selection and optional project access assignment in one step: selecting `super_admin` hides the project access section (super admin has full access); selecting `global_reader` reveals a project assignment panel where one or more projects can be paired with a role (`project_admin`, `contributor`, `reader`) — the modal chains `POST /users` then `POST /projects/:id/access` per assignment; after creation, the "Project Access" button on each row opens `ProjectAccessModal` for further edits.

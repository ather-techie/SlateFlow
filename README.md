...
- **AI features** — gated by `FEATURE_AI=true`; provider-agnostic across Anthropic Claude, Google Gemini, OpenAI, Azure OpenAI, and Ollama:
  ...
  - *Sprint Health Digest* — one click turns velocity, cycle time, capacity, and stalled cards into a stakeholder-ready markdown status report on the Reports page; saved per sprint with regenerate (gated by `FEATURE_AI_CEREMONY_DIGESTS`)
  - *Daily Standup Digest* — "what moved / blocked / watchouts" summary of the last 24h of board activity, comments, and over-capacity assignees, available from the Board header (gated by `FEATURE_AI_CEREMONY_DIGESTS`)
  - *Retrospective Synthesizer* — clusters retro items into themes, drafts action items you can add with one click, and checks whether the previous retro's actions were addressed (gated by `FEATURE_AI_CEREMONY_DIGESTS`)
  - *Acceptance Criteria Generator* — drafts editable Given/When/Then criteria from a story and appends them to the description on confirm (gated by `FEATURE_AI_WRITING_ASSIST`)
  - *Comment Thread Summarizer* — summarizes long card discussions into a recap, decisions, and open questions (gated by `FEATURE_AI_WRITING_ASSIST`)
  - *Smart Assignee & Estimate Suggestions* — suggests the best assignee from skills, current load, and vacations, and story points from comparable completed stories; one-click apply in the card modal (gated by `FEATURE_AI_PLANNING_ASSIST`)
  - *Sprint Planning Assistant* — proposes a sprint scope from backlog priority, dependency order, historical velocity, and member availability, applied via per-story checkboxes (gated by `FEATURE_AI_PLANNING_ASSIST`)
  - *Backlog Grooming Assistant* — flags likely duplicates, vague stories (with suggested rewrites), stale items, and a suggested priority order; every change is an explicit click (gated by `FEATURE_AI_PLANNING_ASSIST`)
  - *Ask Your Project chat* — a streaming, project-scoped assistant ("What's blocking us?", "Who's overloaded?") grounded in live board data and filtered to what the asking user is allowed to see (gated by `FEATURE_AI_PROJECT_CHAT`)

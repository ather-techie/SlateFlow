...
import type { ActivityLog, Card, Comment, Feature, Label, Lane, Sprint, Task, TestCase, TestCaseSummary, TestRun, TestSuite } from '../types'
import { api } from '../api'
...
export default function CardModal({ card, projectId, lanes, sprints, onClose, onUpdate, onDelete }: Props) {
  ...
  async function handleFeatureChange(featureId: number | null) {
    const prev = currentFeatureId
    setCurrentFeatureId(featureId)
    try { onUpdate(await api.updateCard(card.id, { feature_id: featureId })) }
    catch { setCurrentFeatureId(prev) }
  }
  ...
  async function handleToggleTask(task: Task) {
    ...
    setTasks(next)
    syncTaskSummary(next)
    try { await api.cards.updateTask(task.id, { status: nextStatus }) }
    catch {
          ...
          <div className="space-y-4">
            ...
            {features.length > 0 && (
              <div>
                ...
                <select value={currentFeatureId ?? ''} onChange={e => handleFeatureChange(e.target.value ? Number(e.target.value) : null)} className="w-full rounded-lg border border-slate-200 px-3 py-1.5 text-sm text-slate-800 focus:outline-none focus:ring-2 focus:ring-indigo-500 bg-white">
